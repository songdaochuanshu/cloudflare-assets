...
![Workflows](https://img.shields.io/badge/workflows-11%20de--dockerized-blue)
...
│   │   ├── r2-client.ts                 # R2 客户端（AWS Signature V4，已接入 fetchWithRetry）
│   │   ├── cf-api.ts                    # Cloudflare API 客户端（cfFetch 已接入 fetchWithRetry）
│   │   ├── retry.ts                     # fetchWithRetry（超时 + 指数退避 + 抖动 + 幂等判断）
...
│   │   └── send-email.ts                # 旧版发件脚本（已重构为复用 email-template）
...
│   ├── workflows/                       # 11 个业务 workflow（已去 Docker 化）
│   └── _node-ci-bootstrap.yml           # 可复用 workflow（test / build 共享）
└── eslint.config.js                     # flat config（scripts/** 关闭 no-console）
...
> 11 个业务 workflow 全部去 Docker 化，统一使用 `./.github/actions/node-ci` 复合 action；test / build 复用 `_node-ci-bootstrap.yml` 可复用 workflow。
...
| `cdn-list.yml` | 手动 | 列出 CDN 自定义域名 |
| `build.yml` | PR + push main | 仅本地构建（paths 触发） |
| `lint.yml` | PR + push main | ESLint + Prettier |
| `test.yml` | PR + push main | typecheck + vitest |
| `pr-quality.yml` | PR | PR title 规范检查 |
| `security-scan.yml` | push + PR | gitleaks 密钥扫描 |
| `codeql.yml` | push + PR + 每周 | GitHub CodeQL 静态分析 |
