...
```
cloudflare-assets/
├── src/                                 # TypeScript 源码（开发入口）
│   ├── r2/
│   │   └── r2-client.ts                 # 共享 R2 客户端（AWS Signature V4）
│   ├── utils/
...
│   └── types/
│       └── env.d.ts                     # 环境变量 / 公共类型
├── dist/                                # TypeScript 编译产物（git ignore）
│   ├── r2/
│   ├── utils/
│   └── buckets/...
├── cdn/                                 # CDN 配置（待扩展）
...
> **注意**：本项目已迁移到 TypeScript（2026-06-30）。所有脚本源码在 `src/`，运行时跑 `dist/` 产物。
...
> **路径说明**：源码在 `src/buckets/.../*.ts`，运行时跑 `dist/buckets/.../*.js`（由 `npm run build` 编译）。
...
node dist/buckets/songdaochuanshu-static/generate-article.js
```

### 开发流程

1. 改 `src/buckets/.../*.ts` 源码
2. `npm run typecheck`（确保 0 错误）
3. `npm run build`（编译到 `dist/`）
4. 本地 dry-run：`node dist/buckets/.../*.js`
