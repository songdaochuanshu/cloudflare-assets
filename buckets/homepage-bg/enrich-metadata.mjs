...
async function fetchPixivMetadata(pid) {
  try {
...
async function main() {
  ...
  const imagesInfo = JSON.parse(readFileSync(IMAGES_INFO_PATH, 'utf8'));
  
  // 兼容新旧格式
  let allImages = [];
  if (Array.isArray(imagesInfo)) {
    allImages = imagesInfo;
  } else if (imagesInfo.r18 && imagesInfo.normal) {
    allImages = [...imagesInfo.r18, ...imagesInfo.normal];
  }
  ...
  const needEnrich = allImages.filter(
    img => !img.title || !img.author || !img.width || !img.height
  );
  ...
  if (Array.isArray(imagesInfo)) {
    ...
    outputData = {
      r18: allImages.filter(img => img.url.includes('/r18/')),
      normal: allImages.filter(img => img.url.includes('/normal/')),
    };
  }
  ...
  console.log('');
  console.log('⚠️  请手动运行 update-images-info.mjs 上传到 R2');
}
