...
async function main(): Promise<void> {
  if (!ZHIPU_API_KEY) {
    logger.error('[fix-manifest-tags] 错误：未设置 ZHIPU_API_KEY');
    process.exit(1);
  }

  const startTime = Date.now();
  logger.info('[fix-manifest-tags] 扫描 R2 文章...');
  ...
  for (const obj of response.Contents ?? []) {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      logger.error(`[fix-manifest-tags] 读取 ${obj.Key} 失败：${msg}`);
    }
  }

  logger.info(`[fix-manifest-tags] 共找到 ${articles.length} 篇文章`);
  ...
  for (const article of articles) {
    ...
    if (hasCategory && hasTags) {
      ...
      logger.info(`  ⏭️  ${title} — 已有分类[${category}]，跳过 AI`);
    } else {
      // 需要 AI 补充
      const snippet = body
        .replace(/[#*`>\[\]()]/g, ' ')
      ...
      try {
        ...
        tags = result.tags || [];
        logger.info(`  ✅ ${title} → ${category} / ${JSON.stringify(tags)}`);
      } catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        logger.error(`  ❌ ${title} — AI 失败：${msg}`);
...
main().catch((e: Error) => {
  ...
  logger.error(`[fix-manifest-tags] 错误：${e.message}`);
  process.exit(1);
});
