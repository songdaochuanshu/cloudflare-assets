...
⚠️ 非常重要:必须返回纯 JSON,不要用 markdown 代码块包裹,不要有任何额外文字。`;
  ...
  throw lastError ?? new Error('标题筛选重试耗尽');
}
...
async function pickUnusedTopic(): Promise<string> {
  const [cnTitles, usedTitles] = await Promise.all([fetchCnblogsTitles(30), getUsedTitles()]);
...
{"category": "分类名", "tags": ["标签1", "标签2", "标签3"]}`;
  ...
  throw lastError ?? new Error('分类标签重试耗尽');
}
